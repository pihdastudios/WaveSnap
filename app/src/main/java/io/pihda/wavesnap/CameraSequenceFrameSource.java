...
public final class CameraSequenceFrameSource {
  ...
  private void runWorker(CameraEx cameraEx) {
    ...
    try {
      ...
      Logger.info("GestureFrames: starting probe width=" + SOURCE_WIDTH + " height=" + SOURCE_HEIGHT
      ...
      for (int i = 0; i < frames.length; i++) {
        ...
        try {
          ...
          if (totalFrames <= 5 || totalFrames % 100 == 0) {
            Logger.info("GestureFrames: frame=" + totalFrames + " bufferSize=" + frameSize
          ...
          if (!firstFrameReported) {
            ...
            Logger.info(
                "GestureFrames: first frame format=" + format + " payloadSize=" + payloadLength);
            if (listener != null) {
          ...
          if (statsElapsedMs >= STATS_INTERVAL_MS) {
            ...
            Logger.info("GestureFrames: analyticalFps=" + fps
                + " averagePayloadBytes=" + averageBytes + " intervalFrames=" + statsFrames);
  ...
  private void dumpDiagnosticJpeg(ByteBuffer buffer, int length) {
    File outputFile =
        new File(Environment.getExternalStorageDirectory(), "WAVESNAP/GESTURE_PREVIEW_1.JPG");
