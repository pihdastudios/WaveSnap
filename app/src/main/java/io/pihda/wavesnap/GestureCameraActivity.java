 ...
 */
public class GestureCameraActivity extends BaseActivity implements SurfaceHolder.Callback,
    ...
    protected void onResume() {
        ...
        Logger.info("GestureCamera: device model=" + Build.MODEL
        ...
        nativeReady = NativeGestureDetector.initialize(
                CameraSequenceFrameSource.SOURCE_WIDTH,
                CameraSequenceFrameSource.SOURCE_HEIGHT,
                DETECTOR_WIDTH, DETECTOR_HEIGHT);
        nativeInitialized = nativeReady;
        if (nativeReady) {
            Logger.info("GestureCamera: native detector initialized source="
                    + CameraSequenceFrameSource.SOURCE_WIDTH + "x"
                    + CameraSequenceFrameSource.SOURCE_HEIGHT
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        Logger.info("GestureCamera: surfaceChanged format=" + format + " width=" + width + " height=" + height);
    }
    ...
    @Override
    public void onFirstFrame(final CameraSequenceFrameSource source,
            final String format, final int length) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                if (isCurrentFrameSource(source)) {
    ...
    public void onFrame(ByteBuffer buffer, int length, long timestampMs) {
        ...
        if (!nativeReady || (state != GestureState.ARMED
        ...
        if (result == NativeGestureDetector.RESULT_WAVE) {
            Logger.info("GestureCamera: confirmed-wave timestampMs=" + timestampMs
                    + " source=" + ((flags & NativeGestureDetector.FLAG_LOCAL_FLOW) != 0
                            ? "local-flow" : "centroid")
                    + " motionPermille="
                    + NativeGestureDetector.getMotionPermille(packedResult)
                    + " globalPermille="
                    + NativeGestureDetector.getGlobalMotionPermille(packedResult)
                    + " centroid=" + NativeGestureDetector.getCentroidX(packedResult)
                    + "," + NativeGestureDetector.getCentroidY(packedResult)
                    + " areaPermille="
                    + NativeGestureDetector.getCandidateAreaPermille(packedResult));
            if (state == GestureState.ARMED) {
                acceptWave(timestampMs);
            } else {
                Logger.info("GestureCamera: ignored-wave state="
                        + GestureState.name(state) + " timestampMs=" + timestampMs);
        ...
        boolean logMotionDiagnostic = motionDiagnostic
                && timestampMs - lastMotionDiagnosticMs >= 500;
        if (nativeFrameCount <= 5 || nativeFrameCount % 50 == 0 || logMotionDiagnostic) {
            ...
            Logger.info("GestureCamera: nativeFrame=" + nativeFrameCount
                    + " meanLuma=" + NativeGestureDetector.getMeanLuminance(packedResult)
                    + " result=" + result
                    ...
                    + " flags=" + flags
                    + " cameraShift="
                    + NativeGestureDetector.getCameraShiftX(packedResult)
                    + "," + NativeGestureDetector.getCameraShiftY(packedResult)
                    + " localFlow="
                    + NativeGestureDetector.getLocalFlowX(packedResult)
                    + " blocks="
                    + NativeGestureDetector.getLocalFlowBlocks(packedResult)
                    + " processMs=" + processMs
                    + " averageProcessMs=" + averageMs
                    + " jpegBytes=" + length);
        }
    }
    ...
    private void acceptWave(final long timestampMs) {
        ...
        NativeGestureDetector.reset();
        Logger.info("GestureCamera: countdown accepted-wave timestampMs=" + timestampMs
                + " token=" + token);
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
    ...
    private void beginCountdown(final int token) {
        ...
        Logger.info("GestureCamera: countdown start timestampMs=" + countdownStartMs
                + " token=" + token);
        ...
        postAtElapsedDeadline(countdownTwoRunnable,
                countdownStartMs + COUNTDOWN_STEP_MS);
        postAtElapsedDeadline(countdownOneRunnable,
                countdownStartMs + COUNTDOWN_STEP_MS * 2);
        postAtElapsedDeadline(captureRunnable,
                countdownStartMs + COUNTDOWN_STEP_MS * 3);
    }
    ...
    private void scheduleAutomaticShutterRelease() {
        ...
        automaticShutterReleaseRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (activeCamera == null) {
                    Logger.error("GestureCamera: automatic shutter release ignored because preview is not ready");
                    return;
                }
                try {
        ...
        mainHandler.postDelayed(automaticShutterReleaseRunnable,
                AUTOMATIC_SHUTTER_RELEASE_MS);
    }
    ...
    private void updateCooldownStillness(long packedResult, long timestampMs) {
        ...
        int disruptiveFlags = NativeGestureDetector.FLAG_GLOBAL_MOTION
                ...
                | NativeGestureDetector.FLAG_CAMERA_TRANSLATION
                | NativeGestureDetector.FLAG_LOCAL_FLOW;
        boolean still = (flags & disruptiveFlags) == 0
                && NativeGestureDetector.getMotionPermille(packedResult)
                        <= STILL_MOTION_PERMILLE
                && NativeGestureDetector.getGlobalMotionPermille(packedResult)
                        <= STILL_GLOBAL_PERMILLE;
    ...
    protected boolean onShutterKeyDown() {
        ...
        synchronized (stateLock) {
            ...
            if (previousState == GestureState.STOPPED
                    || previousState == GestureState.CAPTURING) {
                Logger.info("GestureCamera: manual shutter ignored state="
                        + GestureState.name(previousState));
                return true;
            }
        ...
        if (previousState == GestureState.COUNTDOWN
                || previousState == GestureState.AUTOFOCUSING) {
    ...
    private void logFailure(String operation, Throwable throwable) {
        ...
        Logger.error("GestureCamera: state=" + GestureState.name(gestureState)
                + " lifecycle=" + (resumed ? "RESUMED" : "STOPPED")
                + " operation=" + operation
