...
public final class NativeGestureDetector {
    ...
    public static boolean initialize(int sourceWidth, int sourceHeight,
    ...
    public static long process(ByteBuffer jpegBuffer, int jpegLength, long timestampMs) {
        ...
        if (jpegBuffer == null || !jpegBuffer.isDirect()
                || jpegLength <= 0 || jpegLength > jpegBuffer.capacity()) {
    ...
    public static int getMeanLuminance(long packedResult) {
        return packedResult < 0 ? -1
                : (int) ((packedResult >> MEAN_LUMA_SHIFT) & MEAN_LUMA_MASK);
    }

    public static int getMotionPermille(long packedResult) {
        return packedResult < 0 ? -1
                : (int) ((packedResult >> MOTION_PERMILLE_SHIFT) & 0x3ff);
    }

    public static int getGlobalMotionPermille(long packedResult) {
        return packedResult < 0 ? -1
                : (int) ((packedResult >> GLOBAL_PERMILLE_SHIFT) & 0x3ff);
    }

    public static int getCentroidX(long packedResult) {
        if ((getFlags(packedResult)
                & (FLAG_CAMERA_TRANSLATION | FLAG_LOCAL_FLOW)) != 0) {
            return -1;
        }
        int value = packedResult < 0 ? 255
                : (int) ((packedResult >> CENTROID_X_SHIFT) & 0xff);
    ...
    public static int getCandidateAreaPermille(long packedResult) {
        return packedResult < 0 ? -1
                : (int) ((packedResult >> AREA_PERMILLE_SHIFT) & 0x3ff);
    }
    ...
    public static int getCameraShiftX(long packedResult) {
        return (getFlags(packedResult) & FLAG_CAMERA_TRANSLATION) == 0 ? 0
    ...
    public static int getLocalFlowX(long packedResult) {
        return (getFlags(packedResult) & FLAG_LOCAL_FLOW) == 0 ? 0
    ...
    private static native long nativeProcess(
            ByteBuffer jpegBuffer, int jpegLength, long timestampMs);
