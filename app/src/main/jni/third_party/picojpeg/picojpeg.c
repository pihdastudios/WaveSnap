...
// The gesture detector is CPU-bound on ARMv5. Keep the entropy-reader helpers
// in the caller so each JPEG symbol does not pay several function calls.
#if defined(__GNUC__)
#define PJPG_INLINE __inline__ __attribute__((always_inline))
#else
#define PJPG_INLINE
#endif
//------------------------------------------------------------------------------
...
{
   return (i > 0 && i <= 15) ? (uint16)(1U << (i - 1)) : 0;
}
//------------------------------------------------------------------------------
static int16 getExtendOffset(uint8 i)
{ 
   return (i > 0 && i <= 15) ? (int16)(1 - (1L << i)) : 0;
};
//------------------------------------------------------------------------------
...
{
   uint8 lookUp = (uint8)(gBitBuf >> 8);
   uint8 lookUpBits = pHuffTable->mLookUpBits[lookUp];
   uint8 i = 0;
   uint8 j;
   uint16 code;
   if (lookUpBits)
   {
      getBits2(lookUpBits);
      return pHuffTable->mLookUp[lookUp];
   }
...
//------------------------------------------------------------------------------
static void huffCreate(const uint8* pBits, HuffTable* pHuffTable, const uint8* pHuffVal)
{
   uint8 i = 0;
   uint8 j = 0;
   ...
   // JPEG's shortest canonical codes dominate normal camera previews. Decode
   // every code up to 8 bits with one indexed load, retaining the canonical
   // path above for the uncommon 9-16 bit codes.
   memset(pHuffTable->mLookUpBits, 0, sizeof(pHuffTable->mLookUpBits));
   for (i = 0; i < 8; i++)
   {
      uint8 codeLength = (uint8)(i + 1);
      uint8 count = pBits[i];
      uint8 valueIndex = pHuffTable->mValPtr[i];
      uint16 firstCode = pHuffTable->mMinCode[i];
      uint8 symbol;
      for (symbol = 0; symbol < count; symbol++)
      {
         uint16 prefix = (uint16)((firstCode + symbol) << (8 - codeLength));
         uint16 suffixCount = (uint16)(1U << (8 - codeLength));
         uint16 suffix;
         for (suffix = 0; suffix < suffixCount; suffix++)
         {
            uint16 entry = (uint16)(prefix + suffix);
            pHuffTable->mLookUp[entry] = pHuffVal[valueIndex + symbol];
            pHuffTable->mLookUpBits[entry] = codeLength;
         }
      }
   }
}
...
static uint8 readDHTMarker(void)
{
   ...
   while (left)
   {
      ...
      huffCreate(bits, pHuffTable, pHuffVal);
   }
...
static void transformBlockReduce(uint8 mcuBlock)
{
   ...
   // WaveSnap's gesture detector consumes JPEG luminance only. Preserve one Y
   // DC sample per 8x8 block and avoid the otherwise wasted RGB conversion.
   if (gReduce == 2)
   {
      switch (gScanType)
      {
         case PJPG_GRAYSCALE:
         case PJPG_YH1V1:
            if (mcuBlock == 0) gMCUBufR[0] = c;
            break;
         case PJPG_YH1V2:
            if (mcuBlock == 0) gMCUBufR[0] = c;
            else if (mcuBlock == 1) gMCUBufR[128] = c;
            break;
         case PJPG_YH2V1:
            if (mcuBlock == 0) gMCUBufR[0] = c;
            else if (mcuBlock == 1) gMCUBufR[64] = c;
            break;
         case PJPG_YH2V2:
            if (mcuBlock < 4) gMCUBufR[mcuBlock * 64] = c;
            break;
      }
      return;
   }
