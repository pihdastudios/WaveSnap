...
int feedTrajectory(const int* x, const int* y, const int* timeMs, int count) {
    ...
    for (int i = 0; i < count; ++i) {
        int flags = 0;
        result = updateWaveTrajectory(x[i], y[i], 50, 100,
                timeMs[i], 64, 36, &flags);
    }
    return result;
}
...
void testFastWaveBothDirections() {
    ...
    resetWaveCompletely();
    expectTrue(feedTrajectory(leftRightLeft, steadyY, timeMs, 4)
            == GESTURE_RESULT_WAVE,
            "270 ms left-right-left wave must be recognized");

    resetWaveCompletely();
    expectTrue(feedTrajectory(rightLeftRight, steadyY, timeMs, 4)
            == GESTURE_RESULT_WAVE,
...
void testTrajectoryRejections() {
    ...
    resetWaveCompletely();
    expectTrue(feedTrajectory(monotonic, steadyY, timeMs, 6)
            != GESTURE_RESULT_WAVE,
            "one-direction motion must not be a wave");

    resetWaveCompletely();
    expectTrue(feedTrajectory(tooSmall, steadyY, timeMs, 6)
            != GESTURE_RESULT_WAVE,
    ...
    for (int i = 0; i < 4; ++i) {
        flags = 0;
        result = updateWaveTrajectory(verticalX[i], verticalY[i], 50, 100,
                timeMs[i], 64, 36, &flags);
        if ((flags & GESTURE_FLAG_TRAJECTORY_REJECTED) != 0) break;
    }
    expectTrue(result != GESTURE_RESULT_WAVE,
            "large vertical drift must not be a wave");
    ...
    expectTrue(result == GESTURE_RESULT_NONE,
            "global motion must cancel a trajectory");
    expectTrue((flags & GESTURE_FLAG_TRAJECTORY_REJECTED) != 0,
...
void testPackedMetrics() {
    ...
    int64_t packed = packMetrics(GESTURE_RESULT_WAVE, 123, 456, 789,
            19, 17, 91, flags);
    expectTrue((packed & 0x3) == GESTURE_RESULT_WAVE,
            "packed result code must round-trip");
    expectTrue((packed & 0x4) != 0,
            "camera translation must use extended result bit");
    expectTrue(((packed >> kMeanLumaShift) & 0xff) == 123,
            "packed luminance must round-trip");
    ...
    expectTrue(((packed >> kFlagsShift) & 0x1f)
            == GESTURE_FLAG_EXPOSURE_CHANGE,
            "packed base flags must exclude extended translation bit");

    packed = packMetrics(GESTURE_RESULT_MOTION, 80, 90, 70,
            44, 5, 20, GESTURE_FLAG_LOCAL_FLOW);
    expectTrue((packed & 0x8) != 0,
            "local optical flow must use second extended result bit");
}
...
void renderSyntheticHand(uint8_t* destination, int handX) {
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            destination[y * width + x] = static_cast<uint8_t>(
                    35 + ((x * 3 + y * 5) & 7));
        }
    }
    for (int y = 10; y < 26; ++y) {
...
void createSyntheticMotionMask() {
    for (size_t i = 0; i < gState.pixelCount; ++i) {
        gState.mask[i] = absoluteDifference(
                gState.grayscale[i], gState.previous[i]) > 8 ? 1 : 0;
    }
}

void testLocalOpticalFlow() {
    expectTrue(Java_io_pihda_wavesnap_NativeGestureDetector_nativeInitialize(
            NULL, NULL, 640, 480, 80, 60) == JNI_TRUE,
    ...
    expectTrue(right.displacementX >= 8,
            "rightward hand flow must retain positive displacement");
    ...
    expectTrue(left.displacementX <= -8,
            "leftward hand flow must retain negative displacement");

    resetWaveCompletely();
    clearFlowTrajectory();
    int flags = 0;
    LocalFlowEstimate flow = {true, 12, 0, 5, 800};
    expectTrue(updateFlowTrajectory(flow, 100, 64, &flags)
            == GESTURE_RESULT_MOTION,
            "first flow vector must begin a trajectory");
    flags = 0;
    expectTrue(updateFlowTrajectory(flow, 190, 64, &flags)
            == GESTURE_RESULT_MOTION,
            "second same-direction vector must extend trajectory");
    flow.displacementX = -14;
    flags = 0;
    expectTrue(updateFlowTrajectory(flow, 280, 64, &flags)
            == GESTURE_RESULT_WAVE,
    ...
    flow.displacementX = -14;
    expectTrue(updateFlowTrajectory(flow, 200, 64, &flags)
            == GESTURE_RESULT_NONE,
            "camera translation must cancel local-flow trajectory");
    expectTrue(gFlow.state == FLOW_IDLE,
            "cancelled local-flow trajectory must return to idle");
    destroyLocked();
}
...
void testInitializationAndJpegDecode() {
    ...
    expectTrue(gState.pixelCount == 4800U,
            "detector pixel count must be 4800");
    destroyLocked();
    destroyLocked();

    expectTrue(Java_io_pihda_wavesnap_NativeGestureDetector_nativeInitialize(
            NULL, NULL, 4096, 4096, 256, 256) == JNI_FALSE,
            "16-bit queue limit must reject 65536 pixels");
    expectTrue(Java_io_pihda_wavesnap_NativeGestureDetector_nativeInitialize(
            NULL, NULL, 32, 24, 4, 3) == JNI_TRUE,
    ...
    expectTrue(gState.currentMeanLuminance > 20
            && gState.currentMeanLuminance < 235,
    ...
    expectTrue((second & 0x3) == GESTURE_RESULT_NONE,
            "identical frame must not create motion");
    expectTrue(((second >> kMotionPermilleShift) & 0x3ff) == 0,
            "identical frame must have zero ROI motion");
    expectTrue(decodeToGrayscale(kSmokeJpeg, 80) < 0,
            "truncated JPEG must fail safely");
    destroyLocked();

    expectTrue(Java_io_pihda_wavesnap_NativeGestureDetector_nativeInitialize(
            NULL, NULL, 31, 24, 4, 3) == JNI_TRUE,
            "mismatch test initialization must succeed");
    expectTrue(decodeToGrayscale(kSmokeJpeg, kSmokeJpegLength)
            == GESTURE_ERROR_JPEG_DIMENSIONS,
...
void testBackgroundTranslation() {
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            gState.previous[y * width + x] = static_cast<uint8_t>(
                    (x * 17 + y * 31 + x * y * 3) & 0xff);
        }
    }
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            int sourceX = x + 3;
            gState.grayscale[y * width + x] = sourceX < width
                    ? gState.previous[y * width + sourceX] : 0;
        }
    }
